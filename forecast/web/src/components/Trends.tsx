  ...
  const spec = useMemo(() => {
    ...
    if (scope.level === "city") {
      ...
      return seriesChartSpec({
        ...
        colors: TEMP_LINE_COLORS, band: ["最低溫", "最高溫"],
      }, data, now, theme);
    }
    const { data, order } = scope.seriesData(fc, TEMP_METRICS[metric]);
    if (data.every((p) => p.value === null)) return null;
    return seriesChartSpec({ yTitle: `${metric} (°C)`, order, zero: true }, data, now, theme);
  }, [scope, fc, now, metric, dark]);
...
export function RainTab({ scope, fc, now }: { scope: Scope; fc: ScopedRow[] | null; now: Date }) {
  ...
  const spec = useMemo(() => {
    ...
    return seriesChartSpec({ yTitle: "降雨機率 (%)", order, zero: true, threshold: RAIN_ALERT, fillZero: true },
      data, now, dark ? DARK_CHART : LIGHT_CHART);
  }, [scope, fc, now, hasData, dark]);
