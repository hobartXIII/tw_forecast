...
SEND_SLOTS = ("08:45", "14:45", "20:45")  # 可選的發送時段；須與資料庫 alert_slot_settings 的 CHECK 一致
...
def is_hit(row: dict, rule: CityRule) -> bool:
    """該縣市已啟用的條件（降雨／低溫／高溫）任一符合。值為 NULL 的欄位不判斷。"""
    rain, tmin, tmax = row.get("rain_probability"), row.get("min_temp"), row.get("max_temp")
    return ((rule.rain_on and rain is not None and rain >= rule.rain)
            or (rule.min_on and tmin is not None and tmin <= rule.min_temp)
            or (rule.max_on and tmax is not None and tmax >= rule.max_temp))


def evaluate_alerts(records: list[dict], settings: AlertSettings, slot: datetime) -> tuple[list[dict], datetime]:
    """回傳 (符合條件的列, 視窗結束時間)；每列多一個 label 欄位（進行中／即將開始），依起點時間排序。"""
    ...
    for row in records:
        ...
        if label and is_hit(row, rule):
            alerts.append({**row, "label": label})
