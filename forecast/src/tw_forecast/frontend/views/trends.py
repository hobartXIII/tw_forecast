...
from tw_forecast.frontend.charts import RAIN_ALERT, TEMP_LINE_COLORS, TEMP_LINE_COLUMNS, SeriesChart
...
def render_temperature_tab(scope: Scope, fc: pd.DataFrame | None, now) -> None:
    ...
    if fc is None:
        ...
        return
    if scope.level == "city":
        st.caption(f"{scope.label}｜最高溫、平均溫、最低溫｜點圖例可強調單一線條，虛線為現在")
