...
用法：
    python tools/make_admin_hash.py             # 互動輸入密碼（輸入兩次確認），印出 INSERT SQL
    python tools/make_admin_hash.py --selftest  # 印出「雜湊相容性檢查」SQL（不含你的密碼），
                                                  # 貼到 SQL Editor 執行，結果應為 true
    python tools/make_admin_hash.py --verify    # 在本機檢查「你輸入的密碼」與「資料庫裡的雜湊」是否相符，
