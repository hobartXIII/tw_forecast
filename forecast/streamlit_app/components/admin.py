...
from .region_data import CITY_ORDER

SLOTS = ("08:45", "14:45", "20:45")          # 可選的發送時段（與資料庫 CHECK 一致）
...
def _translate(exc: Exception, password: str | None) -> AdminError:
...
def _rpc(sb, function: str, params: dict):
    try:
        return sb.rpc(function, params).execute().data
    except Exception as exc:  # 用 from None 切斷原例外，避免其內容出現在 traceback
        raise _translate(exc, params.get("p_password")) from None


def to_dataframe(rows: list[dict]) -> pd.DataFrame:
    """資料庫列 → 表格；依縣市順序（北→中→南→東→離島）排序。"""
...
def get_settings(sb, password: str) -> tuple[pd.DataFrame, dict[str, bool]]:
    """讀取告警設定（密碼在資料庫驗證）。密碼錯誤拋 WrongPassword。"""
    data = _rpc(sb, "admin_get_alert_settings", {"p_password": password})
...
def save_settings(sb, password: str, df: pd.DataFrame, slots: dict[str, bool]) -> dict:
    """儲存告警設定（密碼在資料庫驗證）。驗證不過拋 AdminError；密碼錯誤拋 WrongPassword。"""
    ...
    return _rpc(sb, "admin_save_alert_settings",
