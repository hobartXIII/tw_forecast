...
視窗本身（st.dialog）由 app.py 建立；這裡只放視窗裡的畫面與登入狀態管理，方便單獨測試。
...
def which_dialog(sb, refresh: bool) -> str:
    ...
    if refresh:
        try:
            load(sb, st.session_state["admin_pw"])
        except admin.WrongPassword:
            logout("密碼已失效，請重新登入", defer=False)  # 主流程，立刻顯示
            return "login"
...
def settings_view(sb) -> None:
    """設定視窗：發送時段、22 縣市可編輯表格、儲存／全部啟用／全部關閉／重新載入／登出。"""
    ss = st.session_state
    if not is_logged_in(defer=True):  # 閒置逾時：關閉視窗（提示暫存，整頁重跑後顯示在頁面上）
        st.rerun()
