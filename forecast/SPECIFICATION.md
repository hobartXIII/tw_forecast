...
7. **「立即更新」按鈕**：
   - **目前在 Streamlit 版關閉**（v1.16.0）：「立即更新」改由 Vercel 版前端提供，Streamlit 版（`streamlit` 分支，部署於 Community Cloud）以 `update_gate.MANUAL_UPDATE_ENABLED = False` 關閉：標題列只剩「重新載入資料」「告警設定」兩顆按鈕，不觸發 workflow、不顯示倒數與間隔說明，資料過期的提示改為「請等待下次排程更新」；「最近排程更新／最近手動更新」時間仍顯示。以下規則在開關設為 `True` 時適用。
...
### 10.3 版本紀錄
- **v1.16.1**（`streamlit` 分支）：地圖手勢提示補上 `duration: 1000`：`gestureHandlingOptions` 會整個取代外掛預設值，少了 duration 時電腦上「按住 Ctrl 並滾動滾輪來縮放地圖」的提示一出現就消失（Vercel 版改寫時發現）；測試補一項斷言。
- **v1.16.0**（`streamlit` 分支）：Streamlit 版關閉「立即更新」（改由 Vercel 版提供），以 `update_gate.MANUAL_UPDATE_ENABLED` 開關控制而非刪除程式碼（見 §8.1 第 7 點）；資料過期的提示依開關改寫（`update_gate.stale_hint`）；原本的立即更新測試改在開關開啟下執行，新增 3 項測試（關閉時的按鈕與小字、不顯示倒數、提示文字）。
