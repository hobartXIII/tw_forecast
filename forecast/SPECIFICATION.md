...
8. **告警設定**（管理者；標題列的「⚙️ 告警設定」按鈕，以視窗呈現）：
   - **入口**：標題列三顆按鈕由左至右為「🔄 立即更新」「♻️ 重新載入資料」「⚙️ 告警設定」，沿用相同的按鈕樣式（`width="stretch"`，沒有自訂 CSS）；未設定 Supabase secrets 時停用。**手機版**（視窗寬度 ≤ 640px，與 `st.columns` 自動堆疊的寬度相同）：三顆按鈕收進「☰ 選單」（`st.popover`），點開才顯示三個選項；電腦版不變。實作上兩組按鈕各畫一次（key 以 `_d`、`_m` 區分，容器 key 為 `hdr_desktop`、`hdr_mobile`），由 `style.py` 的 CSS 媒體查詢依寬度只顯示其中一組，兩組的動作相同。
