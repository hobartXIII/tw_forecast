...
9. **玻璃擬態外觀（淺色／深色）**：
   ...
   - **已知限制**：深色主題下地圖底圖仍是淺色圖磚（OpenStreetMap）；使用的 Streamlit 內部選擇器（`.stApp`、`.stDialog`、`.stTabs`，以及頁籤的 `[role="tab"]`／`[aria-selected]`、`.react-aria-SelectionIndicator`，後兩者來自 Streamlit 1.64 採用的 React Aria）在升級版本時須重新檢查外觀。
10. **日期查詢（歷史預報存檔）**：
