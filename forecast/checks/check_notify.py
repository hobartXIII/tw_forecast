"""【真實連線檢查】傳一則範例告警到你的 Telegram，確認 token / chat_id 正確、訊息格式好看。

需要 .env：TELEGRAM_BOT_TOKEN、TELEGRAM_CHAT_ID（可先用 tools/get_telegram_chat_id.py 查詢）。

用法：
    python checks/check_notify.py --dry-run   # 只印出訊息內容，不傳送
    python checks/check_notify.py             # 實際傳送到你的 Telegram
"""
import os
import sys
...
from tw_forecast.backend.notifier import NotifyError, TelegramNotifier, build_alert_text  # noqa: E402
...
def main() -> None:
    ...
    try:
        TelegramNotifier(token, chat_id).send(rich, plain)
